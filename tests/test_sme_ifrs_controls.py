...
def setup(tmp_path):
    ...
    bid=sme_store.create_business(uid,"IFRS Test","Services","Lagos","NGN")
...
def test_supplier_recoverable_vat_posts_to_asset(tmp_path):
    ...
    inv=sme_parties.create_invoice(bid,pid,"SUP-1","2026-09-01","Services",100000,7500,"2026-09-30")
...
def test_controlled_payment_records_then_posts(tmp_path):
    ...
    inv=sme_parties.create_invoice(bid,pid,"INV-1","2026-09-01","Sale",100000,7500,"2026-09-30")
    cash=int(sme_accounting.account_catalog(bid).query("name == 'Main Bank'").iloc[0]["id"])
    payment=ifrs_payment_controls.record_payment_controlled(bid,inv,50000,"2026-09-15",cash,"REF-1")
