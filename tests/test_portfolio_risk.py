...
from analytics.portfolio_risk import (
    align_returns, component_var, correlation_matrix, monte_carlo_var_es,
    portfolio_returns, portfolio_risk, risk_ratios, minimum_variance_weights,
    optimized_portfolio_walk_forward, portfolio_stress_matrix,
)
...
def test_align_returns_and_portfolio_returns():
    ...
    assert align_returns(data).shape == (2, 2)
    assert portfolio_returns(data, {"A": 0.5, "B": 0.5}).iloc[0] == pytest.approx(0.0)


def test_correlation_matrix():
    data = {"A": prices([100, 110, 121, 133.1]), "B": prices([200, 220, 242, 266.2])}
    assert correlation_matrix(data).loc["A", "B"] == pytest.approx(1.0)
...
def test_portfolio_risk_and_component_var():
    ...
    weights = {"A": 0.6, "B": 0.4}
    assert portfolio_risk(data, weights)["observations"] == 4
...
def test_minimum_variance_weights_are_long_only_and_normalized():
    weights = minimum_variance_weights(pd.DataFrame({"A": [0.01, -0.01, 0.01, -0.01], "B": [0.005, -0.005, 0.005, -0.005]}))
...
def test_optimized_walk_forward_is_reproducible_and_oos():
    ...
    t = np.arange(160)
    a = pd.Series(100 * np.cumprod(1 + 0.001 + 0.002 * np.sin(t / 7)), index=index)
    b = pd.Series(100 * np.cumprod(1 + 0.0005 + 0.0015 * np.cos(t / 9)), index=index)
