...
from pages import dashboard, markets, reconciliation, financial_statements, treasury, budget, risk, reports, intelligence, quant_lab, portfolio_risk, research_lab, robustness, data_workspace, garch_lab, liquidity_fx, research, integrated_risk, data_controls, sme_finance
...
page = st.sidebar.radio("Workspace", [
    "📊  Dashboard", "🏢  SME Finance Department", "🧠  Intelligence Centre", "🎯  Integrated Risk Command Centre", "📁  Finance Data Workspace", "🧾  Financial Data Controls", "🧪  Quant Lab", "🔬  Research & Backtesting",
...
if page in {"📊  Dashboard", "🧠  Intelligence Centre", "🎯  Integrated Risk Command Centre", "🧪  Quant Lab", "🔬  Research & Backtesting", "🌍  Emerging Markets Research", "📉  GARCH Volatility Lab", "💱  Liquidity & FX Risk", "🛡️  Risk Robustness Lab", "📈  Markets", "🤖  AI Insights", "🛡️  Risk Monitor", "📐  Portfolio Risk", "📰  News Terminal", "📄  Executive Reports"}:
...
if page == "📊  Dashboard":
...
elif page == "🎯  Integrated Risk Command Centre":
    ...
    research_lab.render()
elif page == "🌍  Emerging Markets Research":
    research.render()
elif page == "📉  GARCH Volatility Lab":
    garch_lab.render(MARKET_SYMBOLS)
elif page == "💱  Liquidity & FX Risk":
    liquidity_fx.render()
    ...
    if news.empty:
        ...
        for _, item in news.iterrows():
            ...
            if url: st.markdown(f"**{headline}**  \n{source} • {published} • [Read article]({url})")
            else: st.markdown(f"**{headline}**  \n{source} • {published}")
            st.divider()
