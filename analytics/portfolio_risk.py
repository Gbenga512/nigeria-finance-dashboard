...
def portfolio_returns(price_map: dict[str, pd.Series], weights: dict[str, float]) -> pd.Series:
    """Compute daily portfolio returns using fixed weights."""
...
def component_var(price_map: dict[str, pd.Series], weights: dict[str, float], confidence: float = 0.95) -> pd.DataFrame:
    """Approximate component VaR using covariance marginal risk contributions."""
    returns = align_returns(price_map)
    if returns.empty:
        return pd.DataFrame()
    ...
    for name in names:
        rows.append({
            "Asset": name,
            "Weight": float(w[name]),
            "Marginal Risk": float(contributions[name] / w[name]) if w[name] != 0 else 0.0,
            "Component Volatility Risk": float(contributions[name]),
            "% of Portfolio Risk": float(contributions[name] / total) if total != 0 else 0.0,
        })
...
def risk_ratios(returns: pd.Series, risk_free_rate: float = 0.0) -> dict[str, float | None]:
    ...
    return {
        "sharpe": float(excess.mean() / clean.std(ddof=1) * np.sqrt(252)) if annual_vol else None,
        "sortino": float(excess.mean() * 252 / downside) if downside else None,
        "calmar": float((annual_return - risk_free_rate) / max_dd) if max_dd else None,
    }
