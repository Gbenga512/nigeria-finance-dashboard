...
def _sortino(returns: pd.Series, periods_per_year: int = 252) -> float:
    ...
    downside_dev = downside.std(ddof=1) if len(downside) > 1 else 0.0
...
def strategy_signal(prices: pd.Series, strategy: str) -> pd.Series:
    ...
    # A signal generated from today's close is applied to tomorrow's return.
...
def select_strategy(train_prices: pd.Series) -> tuple[str, float]:
    """Select one candidate using training data only."""
    scores: dict[str, float] = {}
    for strategy in CANDIDATE_STRATEGIES:
        signal = strategy_signal(train_prices, strategy)
        returns = _clean_prices(train_prices).pct_change().fillna(0.0)
...
def walk_forward_backtest(
    ...
    while start < len(clean):
        ...
        selected, train_sharpe = select_strategy(train)
        ...
        block_rows.append({
            ...
            "Test Return": metrics["cumulative_return"],
            "Test Sharpe": metrics["sharpe"],
            "Test Max Drawdown": metrics["maximum_drawdown"],
            "Benchmark Return": metrics["benchmark_cumulative_return"],
            "Turnover": turnover,
        })
    ...
    return {"summary": summary, "blocks": pd.DataFrame(block_rows), "equity": equity}
