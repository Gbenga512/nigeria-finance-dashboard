...
CREATE TABLE IF NOT EXISTS counterparties (
 ...
 name TEXT NOT NULL, email TEXT, phone TEXT, address TEXT, tax_id TEXT,
...
CREATE TABLE IF NOT EXISTS invoices (
 id INTEGER PRIMARY KEY AUTOINCREMENT, business_id INTEGER NOT NULL,
 party_id INTEGER NOT NULL, invoice_number TEXT NOT NULL, invoice_date TEXT NOT NULL,
 due_date TEXT NOT NULL, invoice_type TEXT NOT NULL CHECK(invoice_type IN ('Sales','Purchase')),
 subtotal REAL NOT NULL CHECK(subtotal >= 0), tax_amount REAL NOT NULL DEFAULT 0 CHECK(tax_amount >= 0),
 total_amount REAL NOT NULL CHECK(total_amount >= 0), status TEXT NOT NULL DEFAULT 'Open' CHECK(status IN ('Draft','Open','Partially Paid','Paid','Void')),
 notes TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
 FOREIGN KEY(business_id) REFERENCES businesses(id) ON DELETE CASCADE,
 FOREIGN KEY(party_id) REFERENCES counterparties(id) ON DELETE RESTRICT,
 UNIQUE(business_id, invoice_number)
);
CREATE TABLE IF NOT EXISTS invoice_payments (
 id INTEGER PRIMARY KEY AUTOINCREMENT, business_id INTEGER NOT NULL, invoice_id INTEGER NOT NULL,
 payment_date TEXT NOT NULL, amount REAL NOT NULL CHECK(amount > 0), account_id INTEGER NOT NULL,
 reference TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
 FOREIGN KEY(business_id) REFERENCES businesses(id) ON DELETE CASCADE,
 FOREIGN KEY(invoice_id) REFERENCES invoices(id) ON DELETE RESTRICT,
 FOREIGN KEY(account_id) REFERENCES accounts(id) ON DELETE RESTRICT
);
CREATE INDEX IF NOT EXISTS idx_counterparty_business ON counterparties(business_id, party_type);
CREATE INDEX IF NOT EXISTS idx_invoice_business_due ON invoices(business_id, due_date, status);
"""
...
def list_counterparties(business_id: int, party_type: str | None = None) -> pd.DataFrame:
    ensure_schema()
    sql="SELECT * FROM counterparties WHERE business_id=? AND active=1"; params=[business_id]
...
def create_counterparty(business_id:int, party_type:str, name:str, **fields) -> int:
    ...
    allowed={"email","phone","address","tax_id","payment_terms_days","credit_limit"}; data={k:v for k,v in fields.items() if k in allowed}
    with connect() as conn:
        try:
            cur=conn.execute("INSERT INTO counterparties(business_id,party_type,name,email,phone,address,tax_id,payment_terms_days,credit_limit) VALUES(?,?,?,?,?,?,?,?,?)",(business_id,party_type,name,data.get("email",""),data.get("phone",""),data.get("address",""),data.get("tax_id",""),int(data.get("payment_terms_days",30)),float(data.get("credit_limit",0))))
...
def create_invoice(business_id:int, party_id:int, invoice_number:str, invoice_date:str, due_date:str, invoice_type:str, subtotal:float, tax_amount:float=0, notes:str="") -> int:
    ...
    with connect() as conn:
        party=conn.execute("SELECT party_type FROM counterparties WHERE id=? AND business_id=? AND active=1",(party_id,business_id)).fetchone()
        expected="Customer" if invoice_type=="Sales" else "Supplier"
        if not party or party["party_type"]!=expected: raise ValueError("Selected party does not match invoice type.")
        cur=conn.execute("INSERT INTO invoices(business_id,party_id,invoice_number,invoice_date,due_date,invoice_type,subtotal,tax_amount,total_amount,notes) VALUES(?,?,?,?,?,?,?,?,?,?)",(business_id,party_id,str(invoice_number).strip(),invoice_date,due_date,invoice_type,subtotal,tax_amount, total,notes))
        conn.commit(); return int(cur.lastrowid)

def invoices_df(business_id:int) -> pd.DataFrame:
    ensure_schema()
    with connect() as conn:
        df=pd.read_sql_query("""SELECT i.*, c.name AS party_name, c.party_type,
        COALESCE((SELECT SUM(p.amount) FROM invoice_payments p WHERE p.invoice_id=i.id),0) AS paid_amount
        FROM invoices i JOIN counterparties c ON c.id=i.party_id WHERE i.business_id=? ORDER BY date(i.due_date),i.id""",conn,params=(business_id,),parse_dates=["invoice_date","due_date"])
    df["outstanding_amount"]=(df["total_amount"]-df["paid_amount"]).round(2)
    return df
...
def record_payment(business_id:int, invoice_id:int, payment_date:str, amount:float, account_id:int, reference:str="") -> int:
    ...
    with connect() as conn:
        ...
        paid=float(conn.execute("SELECT COALESCE(SUM(amount),0) AS total FROM invoice_payments WHERE invoice_id=?",(invoice_id,)).fetchone()["total"])
        outstanding=round(float(inv["total_amount"])-paid,2)
        if amount>outstanding+0.01: raise ValueError(f"Payment exceeds outstanding balance of ₦{outstanding:,.2f}.")
        acct=conn.execute("SELECT id,name FROM accounts WHERE id=? AND business_id=? AND active=1",(account_id,business_id)).fetchone()
        if not acct: raise ValueError("Payment account does not belong to the active business.")
        cur=conn.execute("INSERT INTO invoice_payments(business_id,invoice_id,payment_date,amount,account_id,reference) VALUES(?,?,?,?,?,?)",(business_id,invoice_id,payment_date,amount,account_id,reference))
        new_paid=paid+amount; status="Paid" if abs(new_paid-float(inv["total_amount"]))<0.01 else "Partially Paid"
        conn.execute("UPDATE invoices SET status=? WHERE id=?",(status,invoice_id)); conn.commit()
    accounts={r["name"]:int(r["id"]) for r in connect().execute("SELECT id,name FROM accounts WHERE business_id=?",(business_id,)).fetchall()}
    ar=accounts["Accounts Receivable"] if inv["invoice_type"]=="Sales" else accounts["Accounts Payable"]
    if inv["invoice_type"]=="Sales": lines=[{"account_id":account_id,"debit":amount},{"account_id":ar,"credit":amount}]
    else: lines=[{"account_id":ar,"debit":amount},{"account_id":account_id,"credit":amount}]
    post_journal_entry(business_id,payment_date,f"Payment for {inv['invoice_number']}",lines,reference=reference,source="AR/AP payment")
...
def post_invoice_to_ledger(business_id:int, invoice_id:int) -> int:
    ...
    amount=float(inv["total_amount"]); tax=float(inv["tax_amount"])
    if inv["invoice_type"]=="Sales":
        lines=[{"account_id":accounts["Accounts Receivable"],"debit":amount},{"account_id":accounts["Sales Revenue"],"credit":amount-tax}]
        if tax: lines.append({"account_id":accounts["Tax Payable"],"credit":tax})
    else:
        lines=[{"account_id":accounts["Operating Expenses"],"debit":amount-tax},{"account_id":accounts["Tax Expense"],"debit":tax},{"account_id":accounts["Accounts Payable"],"credit":amount}]
...
def ageing(business_id:int, as_of=None) -> pd.DataFrame:
    ...
    df=invoices_df(business_id); df=df[df["status"].isin(["Open","Partially Paid"])].copy()
    if df.empty: return pd.DataFrame(columns=["Bucket","Amount"])
    df["days_overdue"]=(as_of-df["due_date"]).dt.days
    buckets=[("Current",df["days_overdue"]<=0),("1–30",df["days_overdue"].between(1,30)),("31–60",df["days_overdue"].between(31,60)),("61–90",df["days_overdue"].between(61,90)),("90+",df["days_overdue"]>90)]
