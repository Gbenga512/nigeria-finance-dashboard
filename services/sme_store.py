...
def create_business(user_id: int, name: str, **fields: Any) -> int:
    ...
    columns = ["user_id", "name"] + list(data)
    values = [user_id, clean_name] + [data[k] for k in data]
    with connect() as conn:
        cur = conn.execute(f"INSERT INTO businesses({','.join(columns)}) VALUES({','.join('?' for _ in values)})", values)
        business_id = int(cur.lastrowid)
        conn.executemany("INSERT OR IGNORE INTO accounts(business_id,name,account_type) VALUES(?,?,?)", [(business_id, "Main Bank", "Cash"), (business_id, "Cash on Hand", "Cash"), (business_id, "Accounts Receivable", "Receivable"), (business_id, "Accounts Payable", "Payable")])
        return business_id
...
def bulk_add_transactions(business_id: int, rows: list[dict[str, Any]]) -> int:
    """Post explicitly reviewed rows in one transaction; duplicate import keys are ignored."""
    init_db()
    posted = 0
    allowed = {"category", "account_id", "counterparty", "reference", "payment_method", "tax_amount", "notes", "attachment_path", "import_key", "source"}
    with connect() as conn:
        for row in rows:
            try:
                ...
                data["source"] = row.get("source") or "Bank statement"
                columns = ["business_id", "transaction_date", "description", "amount", "transaction_type"] + list(data)
                values = [business_id, row["transaction_date"], description, amount, transaction_type] + [data[k] for k in data]
                conn.execute(f"INSERT INTO transactions({','.join(columns)}) VALUES({','.join('?' for _ in values)})", values)
                posted += 1
