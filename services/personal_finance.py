...
DEFAULT_SETTINGS = {
    "currency": "NGN", "symbol": "₦", "fiscal_year": date.today().year,
...
def settings() -> dict:
    ...
    numeric = {"opening_cash", "needs_target", "wants_target", "savings_target", "debt_ratio_target", "emergency_months_target"}
    for k, v in rows:
        if k == "fiscal_year": out[k] = int(float(v))
        elif k in numeric: out[k] = float(v)
        else: out[k] = v
    return out
    ...
    with connect() as conn:
        for key, value in values.items():
            if key in DEFAULT_SETTINGS:
                conn.execute("INSERT OR REPLACE INTO personal_settings(key,value) VALUES (?,?)", (key, str(value)))


def categories(category_type: str | None = None) -> pd.DataFrame:
    ensure_schema()
    sql = "SELECT * FROM personal_categories WHERE active=1"
    params = []
    if category_type:
        sql += " AND category_type=?"; params.append(category_type)
    sql += " ORDER BY name"
    with connect() as conn:
...
def add_transaction(transaction_date: str, description: str, amount: float, transaction_type: str,
    ...
    with connect() as conn:
        if not conn.execute("SELECT 1 FROM personal_accounts WHERE id=? AND active=1", (account_id,)).fetchone():
            raise ValueError("Selected personal account does not exist")
        cls = _classification(category)
        cur = conn.execute(
            "INSERT INTO personal_transactions(transaction_date,description,amount,transaction_type,category,classification,account_id,payment_method,reference,notes) VALUES (?,?,?,?,?,?,?,?,?,?)",
...
def transactions(start: str | None = None, end: str | None = None) -> pd.DataFrame:
    ensure_schema()
    sql = "SELECT t.*, a.name AS account_name FROM personal_transactions t JOIN personal_accounts a ON a.id=t.account_id WHERE 1=1"
    params = []
...
def monthly_statement(fiscal_year: int | None = None) -> pd.DataFrame:
    year = fiscal_year or settings()["fiscal_year"]
    df = transactions(f"{year}-01-01", f"{year}-12-31")
    rows = []
    for month in range(1, 13):
        ...
        rows.append({"Month": pd.Timestamp(year, month, 1).strftime("%B"), "Income": income, "Expenses": expense,
                     "Net Savings": income - expense, "Savings/Investment": savings, "Debt Payments": debt,
                     "Savings Rate": (income - expense) / income if income else None})
...
def budget_variance(month: str | None = None) -> pd.DataFrame:
    ...
    with connect() as conn:
        budget = pd.read_sql_query("SELECT category,amount AS budget FROM personal_budgets WHERE month=?", conn, params=[month])
    start = f"{month}-01"; end = (pd.Timestamp(start) + pd.offsets.MonthEnd(1)).strftime("%Y-%m-%d")
    tx = transactions(start, end)
    actual = (tx[tx.transaction_type == "Expense"].groupby("category", as_index=False)["amount"].sum().rename(columns={"amount": "actual"})
              if not tx.empty else pd.DataFrame(columns=["category", "actual"]))
    ...
    out = cats.merge(budget, on="category", how="left").merge(actual, on="category", how="left").fillna(0)
    out["variance"] = out["budget"] - out["actual"]
    out["variance_pct"] = out.apply(lambda r: r.variance / r.budget if r.budget else None, axis=1)
    out["status"] = out.apply(lambda r: "No Budget Set" if r.budget == 0 else ("Over Budget" if r.actual > r.budget else ("Near Limit" if r.actual > 0.9 * r.budget else "Within Budget")), axis=1)
...
def set_budget(month: str, category: str, amount: float) -> None:
    ...
    with connect() as conn:
        conn.execute("INSERT INTO personal_budgets(month,category,amount) VALUES (?,?,?) ON CONFLICT(month,category) DO UPDATE SET amount=excluded.amount", (month, category, amount))


def net_worth(as_of: str | None = None) -> dict:
    ensure_schema(); as_of = as_of or date.today().isoformat()
    with connect() as conn:
        nw = pd.read_sql_query("SELECT item_name,item_type,amount FROM personal_net_worth WHERE as_of_date<=?", conn, params=[as_of])
...
def set_net_worth_item(item_name: str, item_type: str, amount: float, as_of_date: str | None = None) -> None:
    ensure_schema(); item_name = item_name.strip(); amount = float(amount); as_of_date = as_of_date or date.today().isoformat()
    if not item_name: raise ValueError("Asset or liability name is required")
    ...
    except ValueError as exc: raise ValueError("As-of date must be YYYY-MM-DD") from exc
    with connect() as conn:
        conn.execute("INSERT INTO personal_net_worth(item_name,item_type,amount,as_of_date) VALUES (?,?,?,?) ON CONFLICT(item_name,item_type) DO UPDATE SET amount=excluded.amount,as_of_date=excluded.as_of_date", (item_name, item_type, amount, as_of_date))


def financial_ratios(start: str, end: str) -> dict:
    s = settings(); df = transactions(start, end)
    income = float(df.loc[df.transaction_type == "Income", "amount"].sum()) if not df.empty else 0.0
    expense = float(df.loc[df.transaction_type == "Expense", "amount"].sum()) if not df.empty else 0.0
    needs = float(df.loc[(df.transaction_type == "Expense") & (df.classification == "Need"), "amount"].sum()) if not df.empty else 0.0
    wants = float(df.loc[(df.transaction_type == "Expense") & (df.classification == "Want"), "amount"].sum()) if not df.empty else 0.0
    savings = float(df.loc[(df.transaction_type.isin(["Savings", "Investment"])) | (df.classification == "Savings"), "amount"].sum()) if not df.empty else 0.0
    debt = float(df.loc[df.transaction_type == "Debt Payment", "amount"].sum()) if not df.empty else 0.0
    nw = net_worth(end)
    avg_monthly_exp = expense / max(1, (pd.Timestamp(end) - pd.Timestamp(start)).days / 30.4375)
    cash_assets = nw["assets"] if nw["assets"] else s["opening_cash"]
    return {"Savings Rate": savings / income if income else None, "Expense-to-Income Ratio": expense / income if income else None,
            "Needs Ratio": needs / income if income else None, "Wants Ratio": wants / income if income else None,
            "Savings/Investment Ratio": savings / income if income else None, "Debt Repayment Ratio": debt / income if income else None,
            "Emergency Fund Coverage (months)": cash_assets / avg_monthly_exp if avg_monthly_exp else None, "Net Worth": nw["net_worth"]}
...
def spending_by_category(start: str, end: str) -> pd.DataFrame:
    df = transactions(start, end)
    if df.empty: return pd.DataFrame(columns=["category", "amount"])
