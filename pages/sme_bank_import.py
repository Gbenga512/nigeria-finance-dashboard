"""Phase 2 bank statement import: preview, classify, duplicate-check, confirm, post."""
...
from services.sme_store import get_or_create_user, list_businesses, transactions_df, bulk_add_transactions
...
def render() -> None:
    ...
    if not businesses:
        st.info("Create an SME business profile first."); return
    ...
    if uploaded is None:
        st.info("Nothing is posted automatically. Upload a statement to begin the review workflow."); return
    try:
        review=parse_statement(uploaded.getvalue(), uploaded.name)
        review=detect_existing_duplicates(review, transactions_df(business_id))
        review=suggest_categories(review, transactions_df(business_id))
    except (ValueError, ImportError) as exc:
        st.error(str(exc)); return
    st.session_state["sme_import_review"]=review
    ...
    invalid=int((~edited["valid"]).sum()); duplicates=int((edited["existing_duplicate"] | edited["duplicate"]).sum())
    c1,c2,c3=st.columns(3); c1.metric("Rows",len(edited)); c2.metric("Invalid",invalid); c3.metric("Duplicate flags",duplicates)
    if invalid: st.warning("Invalid rows will not be posted.")
    if duplicates: st.warning("Duplicate-flagged rows are excluded from posting unless you remove the duplicate condition from the review data.")
    if st.button("Confirm & Post Reviewed Transactions", type="primary", use_container_width=True):
        ...
        posted=bulk_add_transactions(business_id, rows)
