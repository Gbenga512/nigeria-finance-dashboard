...
def render() -> None:
    ...
    uploaded = st.file_uploader(
        ...
        type=["csv", "xlsx", "xls"],
        help="CSV or Excel only in this release. PDF extraction will be added after a reliable parser is selected.",
    )
    if uploaded is None:
    ...
    editable = review[[
        "transaction_date", "description", "amount", "transaction_type", "reference",
        "suggested_category", "direction_review_required", "existing_duplicate", "duplicate", "valid",
    ]].copy().rename(columns={"suggested_category": "category"})
    ...
    direction_review = int(edited["transaction_type"].isna().sum() + (edited["transaction_type"].astype(str).str.strip() == "").sum())
    duplicates = int((edited["existing_duplicate"] | edited["duplicate"]).sum())
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Rows", len(edited))
    c2.metric("Invalid", invalid)
    c3.metric("Direction review", direction_review)
    c4.metric("Duplicate flags", duplicates)
    if invalid:
    ...
    postable = (
        edited["valid"]
        & edited["transaction_type"].isin(["Income", "Expense", "Transfer"])
        & ~edited["existing_duplicate"]
        & ~edited["duplicate"]
    )
